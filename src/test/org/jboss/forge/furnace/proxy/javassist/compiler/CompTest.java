...
import org.jboss.forge.furnace.proxy.javassist.compiler.TypeChecker;
...
public class CompTest extends TestCase {
    ...
    public void testCast() throws Exception {
        Javac jc = new Javac(sloader.get("org.jboss.forge.furnace.proxy.javassist.compiler.Print"));
        jc.compileStmnt(
              "{((org.jboss.forge.furnace.proxy.javassist.compiler.Advice)"
            + "  (new org.jboss.forge.furnace.proxy.javassist.compiler.HelloAspect().getAdvice().get(0)))"
            + "  .print();}");
    }

    public void testStaticMember() throws Exception {
        String src = "org.jboss.forge.furnace.proxy.javassist.compiler.Print#k = 3;";
        Parser p = new Parser(new Lex(src));
        SymbolTable stb = new SymbolTable();
        Stmnt s = p.parseStatement(stb);
        Expr expr = (Expr)s.getLeft().getLeft();
        assertEquals('#', expr.getOperator());
        assertEquals("org.jboss.forge.furnace.proxy.javassist.compiler.Print",
    ...
    public void testRecordLocalVar() throws Exception {
        Javac jv = new Javac(sloader.get("org.jboss.forge.furnace.proxy.javassist.compiler.Print"));
    ...
    public static void main(String[] args) {
        ...
        junit.awtui.TestRunner.main(new String[] {
            "org.jboss.forge.furnace.proxy.javassist.compiler.CompTest" });
    }
